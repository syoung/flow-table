...
use FindBin qw($Bin);
use lib "$Bin/../../";
use Table::DBase::Main;

class Table::DBase::MySQL extends Table::DBase::Main {
...
has 'dbhost'	=> ( isa => 'Str|Undef', is => 'rw', default => 'localhost' );
...
method initialise ($args) {
	...
	$dbhost = $self->dbhost() if not defined $dbhost or not $dbhost;
	$dbhost = "localhost" if not defined $dbhost or not $dbhost;
	...
	my $dsn = "DBI:mysql:$database:$dbhost;mysql_local_infile=1";
...
=cut
	...
	my $hasTable = $self->hasTable($table);
...
method createTable ($sqlfile) {
    ...
    my $query = $self->getFileContents($sqlfile);    
...
#### RETURN AN ARRAY OF THE TABLES FIELDS OR '' IF NOT DEFINED
	my $sql 	=	$self->util()->fileContents($sqlfile);
...
#### CHECK IF A TABLE HAS A PARTICULAR FIELD
	...
	my $fields = $self->fieldsToArray($table);
...
#### LOAD DATA FROM .TSV FILE INTO TABLE
	...
	if ( not $count ) {   print "Table::DBase::MySQL    DID NOT LOAD!\n";    }
	$self->dbh()->{RaiseError} = 1; # ENABLE TERMINATION ON ERROR
}
...
method createDatabase ($database) {
	...
	my $remotecall = $drh->func('createdb', $database, 'localhost', $dbuser, $dbpassword, 'admin');
...
print "Table::DBase::MySQL::useDatabase    database: $database\n";
...
#### RETURN 1 IF A DATABASE EXISTS WITH THE GIVEN NAME, OTHERWISE RETURN 0
	...
	die "Table::DBase::MySQL::isDatabase    user not defined\n" if not defined $user;
	die "Table::DBase::MySQL::isDatabase    password not defined\n" if not defined $password;
...
method dropDatabase ($database) {
    ...
    my $drh = DBI->install_driver("mysql");
	my $remotecall = $drh->func('dropdb', $database, 'localhost', $dbuser, $dbpassword, 'admin');
	$self->logNote("remotecall", $remotecall);
