...
class Table::DBase::SQLite extends Table::DBase::Main {
...
=cut
    $self->logDebug("Table::DBase::SQL:ite::loadNoDups(table, bsvfile)");
    ...
    die "Table::DBase::SQL:ite::loadNoDups    Fields not defined for table: $table\n" if not defined $fields;
    ...
    if ( defined $keys )
    {
        ...
        foreach my $key ( @$keys )
        {
            ...
            if ( not $found )
            {
                die "Table::DBase::SQL:ite::loadNoDups    key $key not found in fields: @$fields\n";
            }
        }
        ...
        open(FILE, $sortedfile) or die "Table::DBase::SQL:ite::loadNoDups    Can't open input file: $sortedfile, error: $!\n";
        $/ = "\n";
        while ( <FILE> )
        {
    ...
    else
    {
        ...
        close(FILE);
        close(OUTFILE);
        print "Table::DBase::SQL:ite::loadNoDups    No dups file printed:\n\n$nodupsfile\n\n";
    }
...
=head2
  ...
  PURPOSE
    ...
    OVERRIDE field METHOD IN Table::DBase::Main - USE SQLITE CURRENT_TIMESTAMP
...
=cut
    ...
    foreach my $field  ( @$fields ) {
      if ( $field eq "datetime" 
        || $field eq "installed" ) {
        my $now = $self->query("SELECT CURRENT_TIMESTAMP");
...
method importFile ( $table, $file ) {
  ...
  my $dbfile = $self->dbfile();
  my $command = qq{sqlite3 $dbfile -separator '\t' ".import $file $table"};
  $self->logDebug("command", $command);
  `$command`;
...
=cut
    $self->logNote("Table::DBase::SQLite::query(query)");
    $self->logNote("query", $query);
    my $response = $self->dbh()->selectall_arrayref($query);
    $self->logNote("response", $response);
...
=cut
  ...
  if ( $fieldstring =~ /^\*$/ ) {
    ...
    print "Table::DBase::SQLite::queryhash    fields not defined. Exiting\n" and return if not defined $fields;
...
=cut
    $self->logDebug("Table::DBase::SQLite::fieldsToTsv(fields, hash)");
    $self->logDebug("fields", $fields);
    $self->logDebug("hash", $hash);

    if ( ref($fields) ne "" )
    {
        print "Table::DBase::SQLite::fieldsToTsv    Fields not a scalar value (i.e., not a CSV string)\n";
        $self->logDebug("ref(fields): " . ref($fields));
        print "Table::DBase::SQLite::fieldsToTsv    Quitting...\n";
        return;
    }
    $fields =~ s/\s//g;
    my $tsvline = '';
